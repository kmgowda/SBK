...
The deployment description above is the default `-compactruntimecopy false`
flow. With `-compactruntimecopy true`, the same Gradle build contract instead
selects the requested driver's transitive SBK closure and a runtime-only Java
image generated with `jlink`. Gradle's independent
