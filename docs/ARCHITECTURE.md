...
Remote execution uses an immutable, content-addressed runtime bundle rather than independent mutable SBK and Java directory copies. GEM first validates the installed SBK pathing JAR and all manifest dependencies, then packages the complete distribution plus the controller JDK when `javacopy=true`. The identity includes file bytes, symbolic links, and executable state. Controller and nodes must be a homogeneous Linux or macOS cluster with the same `amd64` or `arm64` architecture. Each node verifies the archive SHA-256, platform descriptor, and per-file SHA-256 manifest in a unique staging directory before an atomic rename. An exact valid identity is reused; incomplete staging data is cleaned and is never launched. With `javacopy=false`, the same SBK bundle is used but every node must supply a matching executable `java` and `javac`. Each launch exports the selected node-specific `SBK_JAVA_HOME`, and remote SBK uses `GrpcLogger` to reach the embedded SBM instance.
