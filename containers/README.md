...
- Enough local disk for the selected JDK base layers and BuildKit cache
...
Override the local repository prefix and pinned runtime base when preparing:

```bash
./gradlew verifyContainerBuild \
  -PcontainerDrivers=minio \
  -PcontainerRepository=registry.example.com/perf/sbk \
  -PcontainerRuntimeImage=eclipse-temurin:25-jdk-ubi10-minimal
```
...
- Keep the base image on Java 25 and pin it by digest in controlled CI.
